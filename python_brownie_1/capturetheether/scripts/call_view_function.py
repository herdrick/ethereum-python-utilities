...
@click.option("--transact", type=bool, default="False", help="Initiate a transaction which will be executed on the blockchain (rather than a calling a simple view function whose execution will only happen on the node you are connecting to).")
...
def call_or_transact(contract_address, function_name, abi_file, network, transact, ethereum_public_key, ethereum_private_key, nonce):
    ...
    """
    if transact:
        assert ethereum_public_key and ethereum_private_key, "If --transact then you must specify ethereum_private_key and ethereum_public_key or set those env vars."
    ...
    ic(ethereum_public_key)
    abi = json.load(abi_file)
    contract_instance = w3.eth.contract(address=contract_address, abi=abi)
    ic(contract_instance)
